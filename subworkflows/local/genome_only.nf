...
include { BUSCO as BUSCO_GENOME_ONLY } from '../../modules/nf-core/busco/main'
...
workflow GENOME_ONLY {
    ...
    ch_versions = ch_versions.mix(BUSCO_GENOME_ONLY.out.versions.first())
    ...
    GAWK(
        BUSCO_GENOME_ONLY.out.batch_summary,
        [],
        false,
    )
    ...
    ch_full_table = BUSCO_GENOME_ONLY.out.full_table
    ...
    // Prepare data
    ch_busco_proteins = BUSCO_GENOME_ONLY.out.single_copy_faa
        | flatMap { meta, faas ->
    ...
    BUSCO_TSV_TO_GFF(
        BUSCO_GENOME_ONLY.out.busco_dir
    )
    ...
    busco_short_summaries = BUSCO_GENOME_ONLY.out.short_summaries_txt // channel: [ val(meta), [txt] ]
